    ...
    public function index()
    {
        ...
        if (request('no_rfid')) {
            ...
            if ($cekrfid->count() > 0) {
                ...
                $jam_now = Carbon::now()->format('H:i:s');
                $jam_now_string = Carbon::now()->toDateTimeString();
                $jadwal_ujian = JadwalUjian::select()->wheredate('tgl_ujian', $date_now)->wheretime('jam_mulai_ujian', '<=', $jam_now)->wheretime('jam_berakhir_ujian', '>=', $jam_now)->where('kelas', $mahasiswa->kelas)->where('tahun', $mahasiswa->tahun_masuk)->get();
                if ($jadwal_ujian) {
                    if ($jadwal_ujian->first()) {
                        $masuk = $jadwal_ujian->first()->jam_mulai_ujian;
                        $tgl_ujian = Carbon::parse($jadwal_ujian->first()->tgl_ujian);
